# ============================================================
# KCleaner - Version 1.0 by Newf276 (2024)
# ------------------------------------------------------------
# Clean up Kodi
# ------------------------------------------------------------
            ...
            if jj["file"][:9] != "addons://" and jj["file"][:6] != "rss://" and jj["file"][:6] != "ftp://" \
                ...
                and jj["file"][:10] != "musicdb://" and jj["file"][:7] != "cdda://":
                    ...
                    if xbmcvfs.exists(afg):
                        xbmc.log("BCLEANER >> SOURCE PATHS (" + k +") >> " + afg + " >> ok")
                    else:
                        xbmc.log("BCLEANER >> SOURCE PATHS (" + k +") >> " + afg + " >> error")
...
def CalcDeleted():
    ...
    for i, line in enumerate(msizes):
        xbmc.log("BCLEANER >> CALCULATED SAVINGS >> " + str(line))
                ...
                if get_filename(f) == "settings" or root == userDataPath:
                    ...
                    if os.path.isfile(pout):
                        try:
                            ...
                            xbmc.log("BCLEANER >> COULDN'T DELETE OLD _NEW FILE")
                    ...
                    for line in fp:
                        if userDataPath in line:
                            ...
                            xbmc.log("BCLEANER >> COMPACTING PATH: " + root + "\\" + f)
                            wasChanged = True
                        else:
                            newline = line
                    ...
                    if wasChanged:
                        ...
                        try:
                            ...
                            xbmc.log("BCLEANER >> COULDN'T DELETE OR RENAME ORIGINAL FILE")
                    else:
                        try:
                            os.remove(pout)
                        except Exception:
                            xbmc.log("BCLEANER >> COULDN'T DELETE NEW FILE")
...
def CleanTextures(iMode):
    ...
    try:
        ...
        xbmc.log("BCLEANER >> SQL ERROR IN Textures13: " + str(e))
        data = None
    ...
    for d in data:
        ...
        if not os.path.isfile(thumbPath):
            try:
                ...
                if booDebug:
                    xbmc.log("BCLEANER >> DELETED RECORD FROM DB: " + str(thumbPath))

                counter += 1
            except Exception as e:
                xbmc.log("BCLEANER >> SQL ERROR IN Textures13 DELETING ID: " + str(recID) + " >> " + str(e))
        ...
        if fileName.startswith("http://") or fileName.startswith("https://") or fileName.startswith("image://") or fileName.endswith("/transform?size=thumb"):
            ...
            if not os.path.isfile(fileName):
                try:
                    ...
                    if booDebug:
                        xbmc.log("BCLEANER >> DELETED RECORD FROM DB: " + str(fileName))
        ...
        if entry[3] in cleanIt and not entry[4]:
            ...
            if os.path.exists(clear_cache_path):
                            ...
                            if entry[3] == 'packages' and ignore_packages > 0:
                                if f in plist:
                                    try:
                                        ...
                                        TotalfileSize += fileSize
                                        if booDebug:
                                            xbmc.log("BCLEANER >> DELETED >>" + f)
                                    except Exception as e:
                                        xbmc.log("BCLEANER >> CAN NOT DELETE FILE >>" + f + "<< ERROR: " + str(e))
                            ...
                            for d in dirs:
                                if os.path.join(root, d) != anigPath and os.path.join(root, d) != arccPath:
                                    try:
                                        ...
                                        if booDebug:
                                            xbmc.log("BCLEANER >> DELETED >>" + d)
                                    except Exception as e:
                                        xbmc.log("BCLEANER >> CAN NOT DELETE FOLDER >>" + d + "<< ERROR: " + str(e))
                    else:
                        pass
                ...
                if TotalfileSize > 0:
                    ...
                    xbmc.log("BCLEANER >> CLEANING >> " + mess)
...
def CompactDatabases(iMode):
    ...
    if os.path.exists(dbPath):
        ...
        for f in files:
            ...
            xbmc.log("BCLEANER >> COMPACTED DATABASE >>" + f)
...
def getLocalRepos():
    ...
    for f in repos:
        ...
        if booDebug:
            xbmc.log("BCLEANER >> INSTALLED REPOS >>" + f["addonid"])
...
def getLocalAddons():
    ...
    for f in addons:
        if f["type"] != "xbmc.addon.repository":
            ...
            if booDebug:
                xbmc.log("BCLEANER >> INSTALLED ADDONS >>" + f["addonid"])
    ...
    for d in addonData:
        ...
        if d not in installedAddons:
            ...
            try:
                ...
                xbmc.log("BCLEANER >> DELETING UNUSED ADDON DATA FOLDER >>" + fullName)
                deleted += 1
            except Exception as e:
                xbmc.log("BCLEANER >> ERROR DELETING UNUSED ADDON DATA FOLDER: " + str(e))
...
def getJson(method, param1, param2, retname):
    ...
    if 'result' in py and retname in py['result']:
        ...
        if booDebug:
            xbmc.log("BCLEANER >> READ SYSTEM SETTING >> " + method + ":" + param1 + "."+ param2 + " >> " + str(a) )
...
def ProcessAddons(iMode):
    ...
    for a in AddonsInstalled:
        ...
        if a in AddonsInRepo:
            xbmc.log("BCLEANER >> ADDON >> " + a + " >> FOUND")
        else:
            xbmc.log("BCLEANER >> ADDON >> " + a + " >> IS IN NO REPOSITORY")
...
def ProcessRepos(iMode):
    ...
    for r in repos:
        ...
        xbmc.log("BCLEANER >> PROCESSING REPO >>" + r)
        ...
        if len(AddonsInRepo) == 0:
            xbmc.log("BCLEANER >> REPO >> " + r + " >> " + repoxml + " >> EMPTY OR ERROR")
            ...
            if len(similar) == 0:
                xbmc.log("BCLEANER >> REPO >> " + r + " >> CONTAINS NO LOCAL ADDONS")
                mess1 = __addon__.getLocalizedString(30121)             # CONTAINS NO LOCAL ADDONS
                strEndMessage += r + " [B][COLOR red]" + mess1 + "[/B][/COLOR]\n"
            else:
                if booDebug:
                    for i in similar:
                        xbmc.log("BCLEANER >> REPO >> " + r + " >> CONTAINS >>" + i)
...
def GetAddonsInRepo(netxml, repo):
    ...
    if os.path.exists(netxml):
        if booDebug:
            xbmc.log("BCLEANER >> LOCAL XML EXISTS >>" + netxml)

        repopath = getRepoPath(netxml)

        if not repopath and booDebug:
            xbmc.log("BCLEANER >> INFO TAG NOT FOUND IN REPO >>" + netxml)
        ...
        for ri in repopath:
            ...
            for addon in id:
                ...
                if booDebug:
                    xbmc.log("BCLEANER >> ADDON >>" + addon)

    else:
        if booDebug:
            xbmc.log("BCLEANER >> LOCAL XML DOES NOT EXIST >>" + netxml)
...
def getRepoPath(xmlFile):
    ...
    for r in infoTag:
        try:
            ...
            if booDebug:
                xbmc.log("BCLEANER >> REPO PATHS >>" + r.childNodes[0].nodeValue)
        except Exception as e:
            xbmc.log("BCLEANER >> ERROR REPO PATHS >>" + str(e).encode('utf-8'))
...
#        f.close()
    ...
    try:
        ...
        xbmc.log("BCLEANER >> ERROR PARSING REPO >> " + str(e))
        return ""
    ...
    for r in infoTag:
        ...
        if addon != repo:
            ...
            if booDebug:
                xbmc.log("BCLEANER >> REPO-ADDON-LIST >>" + addon)
...
__addon__ = xbmcaddon.Addon(id='script.bcleaner')
...
xbmc.log("BCLEANER >> STARTED VERSION %s" % (__version__))
...
xbmc.log('BCLEANER SERVICE >> RUNNING APP...' + str(xbmcgui.Window(xbmcgui.getCurrentWindowId()).getProperty('bcleaner')))
