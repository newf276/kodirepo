...
def GetSetting():
    ...
    xbmc.log('BCLEANER SERVICE >> SETTINGS CHANGED >> SERVICE RUN: ' + str(booBackgroundRun))
    xbmc.log('BCLEANER SERVICE >> SETTINGS CHANGED >> RUN EVERY DAYS: ' + str(lastrundays))
...
if __name__ == '__main__':
    xbmc.log("BCLEANER SERVICE >> STARTED VERSION %s" % (__version__))
    ...
    if booBackgroundRun:
        xbmc.log("BCLEANER SERVICE >> SERVICE INIT >> LAST RUN " + str(time_difference_days) + " DAYS AGO, SET TO RUN EVERY " + str(lastrundays) + " DAYS, WITH DELAY OF " + str(autostart_delay) + " MINUTE(S)")
        ...
        if time_difference_days > lastrundays or lastrundays == 0:
            ...
            if __addon__.getSetting('lock') != 'true':
                ...
                xbmc.log('BCLEANER SERVICE >> RUNNING AUTO...')
                AutoClean()
                __addon__.setSetting('auto_lastrun', str(int(round(time.time()))))
                __addon__.setSetting('lock', 'false')
    else:
        xbmc.log("BCLEANER SERVICE >> SERVICE OFF")
    ...
    while True:
        if monitor.waitForAbort(2):    # Sleep/wait for abort
            xbmc.log('BCLEANER SERVICE >> EXIT')
            ...
            if booBackgroundRun:
                ...
                if iCounter > 1800:
                    ...
                    xbmc.log("BCLEANER SERVICE >> LAST RUN " + str(time_difference_days) + " DAYS AGO, SET TO RUN EVERY " + str(lastrundays) + " DAYS (NOW: " + str(date_now) + ")")
                    ...
                    if time_difference_days > lastrundays:
                        if __addon__.getSetting('lock') != 'true':
                            ...
                            __addon__.setSetting('lock', 'false')
                            xbmc.log('BCLEANER SERVICE >> END AUTO...')
