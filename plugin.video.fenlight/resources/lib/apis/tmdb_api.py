...
from modules.kodi_utils import make_session, tmdb_default_api, tmdb_dict_removals, remove_keys
# from modules.kodi_utils import logger

EXPIRY_4_HOURS, EXPIRY_2_DAYS, EXPIRY_1_WEEK = 4, 48, 168
tmdb_api_key = tmdb_default_api
...
def tmdb_movie_keyword_results(tmdb_id, page_no):
	...
	url = '%s/discover/movie?api_key=%s&language=en&with_keywords=%s&page=%s' % (base_url, tmdb_api_key, tmdb_id, page_no)
...
def tmdb_tv_keyword_results(tmdb_id, page_no):
	...
	url = '%s/discover/tv?api_key=%s&language=en&with_keywords=%s&page=%s' % (base_url, tmdb_api_key, tmdb_id, page_no)
	return lists_cache_object(get_data, string, url, json=False, expiration=EXPIRY_2_DAYS)
...
def tmdb_movies_popular(page_no):
	...
	url = '%s/movie/popular?api_key=%s&language=en&region=US&with_original_language=en&page=%s' % (base_url, tmdb_api_key, page_no)
	return lists_cache_object(get_data, string, url, json=False, expiration=EXPIRY_2_DAYS)

def tmdb_movies_popular_today(page_no):
	string = 'tmdb_movies_popular_today_%s' % page_no
	url = '%s/trending/movie/day?api_key=%s&language=en&region=US&with_original_language=en&page=%s' % (base_url, tmdb_api_key, page_no)
	return lists_cache_object(get_data, string, url, json=False, expiration=EXPIRY_2_DAYS)

def tmdb_movies_blockbusters(page_no):
	string = 'tmdb_movies_blockbusters_%s' % page_no
	url = '%s/discover/movie?api_key=%s&language=en&region=US&with_original_language=en&sort_by=revenue.desc&page=%s' % (base_url, tmdb_api_key, page_no)
	return lists_cache_object(get_data, string, url, json=False, expiration=EXPIRY_2_DAYS)

def tmdb_movies_in_theaters(page_no):
	string = 'tmdb_movies_in_theaters_%s' % page_no
	url = '%s/movie/now_playing?api_key=%s&language=en&region=US&with_original_language=en&page=%s' % (base_url, tmdb_api_key, page_no)
	return lists_cache_object(get_data, string, url, json=False, expiration=EXPIRY_2_DAYS)
...
def tmdb_movies_upcoming(page_no):
	...
	url = '%s/discover/movie?api_key=%s&language=en&region=US&with_original_language=en&release_date.gte=%s&release_date.lte=%s&with_release_type=3|2|1&page=%s' \
...
def tmdb_movies_latest_releases(page_no):
	...
	url = '%s/discover/movie?api_key=%s&language=en&region=US&with_original_language=en&release_date.gte=%s&release_date.lte=%s&with_release_type=4|5|6&page=%s' \
...
def tmdb_movies_premieres(page_no):
	...
	url = '%s/discover/movie?api_key=%s&language=en&region=US&with_original_language=en&release_date.gte=%s&release_date.lte=%s&with_release_type=1|3|2&page=%s' \
							% (base_url, tmdb_api_key, previous_date, current_date, page_no)
	return lists_cache_object(get_data, string, url, json=False, expiration=EXPIRY_2_DAYS)

def tmdb_movies_genres(genre_id, page_no):
	string = 'tmdb_movies_genres_%s_%s' % (genre_id, page_no)
	url = '%s/discover/movie?api_key=%s&with_genres=%s&language=en&region=US&with_original_language=en&release_date.lte=%s&page=%s' \
...
def tmdb_movies_languages(language, page_no):
	...
	url = '%s/discover/movie?api_key=%s&language=en&with_original_language=%s&release_date.lte=%s&page=%s' \
...
def tmdb_movies_certifications(certification, page_no):
	...
	url = '%s/discover/movie?api_key=%s&language=en&region=US&with_original_language=en&certification_country=US&certification=%s&release_date.lte=%s&page=%s' \
							% (base_url, tmdb_api_key, certification, get_current_date(), page_no)
	return lists_cache_object(get_data, string, url, json=False, expiration=EXPIRY_2_DAYS)

def tmdb_movies_year(year, page_no):
	string = 'tmdb_movies_year_%s_%s' % (year, page_no)
	url = '%s/discover/movie?api_key=%s&language=en&region=US&with_original_language=en&certification_country=US&primary_release_year=%s&page=%s' \
...
def tmdb_movies_decade(decade, page_no):
	...
	url = '%s/discover/movie?api_key=%s&language=en&region=US&with_original_language=en&certification_country=US&primary_release_date.gte=%s' \
...
def tmdb_movies_networks(network, page_no):
	...
	url = '%s/discover/movie/?api_key=%s&watch_region=US&with_watch_providers=%s&vote_count.gte=100&page=%s' % (base_url, tmdb_api_key, network, page_no)
	return lists_cache_object(get_data, string, url, json=False, expiration=EXPIRY_2_DAYS)

def tmdb_movies_recommendations(tmdb_id, page_no):
	string = 'tmdb_movies_recommendations_%s_%s' % (tmdb_id, page_no)
	url = '%s/movie/%s/recommendations?api_key=%s&language=en&region=US&with_original_language=en&page=%s' % (base_url, tmdb_id, tmdb_api_key, page_no)
	return lists_cache_object(get_data, string, url, json=False, expiration=EXPIRY_2_DAYS)

def tmdb_movies_search(query, page_no):
	string = 'tmdb_movies_search_%s_%s' % (query, page_no)
	url = '%s/search/movie?api_key=%s&language=en&include_adult=true&query=%s&page=%s' % (base_url, tmdb_api_key, query, page_no)
	return lists_cache_object(get_data, string, url, json=False, expiration=EXPIRY_2_DAYS)

def tmdb_movies_companies(company_id, page_no):
	string = 'tmdb_movies_companies_%s_%s' % (company_id, page_no)
	url = '%s/discover/movie?api_key=%s&language=en&region=US&with_original_language=en&certification_country=US&with_companies=%s&page=%s' \
...
def tmdb_tv_popular_today(page_no):
	...
	url = '%s/trending/tv/day?api_key=%s&language=en&region=US&with_original_language=en&page=%s' % (base_url, tmdb_api_key, page_no)
	return lists_cache_object(get_data, string, url, json=False, expiration=EXPIRY_2_DAYS)

def tmdb_tv_premieres(page_no):
	current_date, previous_date = get_dates(31, reverse=True)
	string = 'tmdb_tv_premieres_%s' % page_no
	url = '%s/discover/tv?api_key=%s&language=en&region=US&with_original_language=en&first_air_date.gte=%s&first_air_date.lte=%s&page=%s' \
							% (base_url, tmdb_api_key, previous_date, current_date, page_no)
	return lists_cache_object(get_data, string, url, json=False, expiration=EXPIRY_2_DAYS)

def tmdb_tv_airing_today(page_no):
	string = 'tmdb_tv_airing_today_%s' % page_no
	url = '%s/tv/airing_today?api_key=%s&language=en&region=US&with_original_language=en&page=%s' % (base_url, tmdb_api_key, page_no)
	return lists_cache_object(get_data, string, url, json=False, expiration=EXPIRY_2_DAYS)

def tmdb_tv_on_the_air(page_no):
	string = 'tmdb_tv_on_the_air_%s' % page_no
	url = '%s/tv/on_the_air?api_key=%s&language=en&region=US&with_original_language=en&page=%s' % (base_url, tmdb_api_key, page_no)
	return lists_cache_object(get_data, string, url, json=False, expiration=EXPIRY_2_DAYS)
...
def tmdb_tv_genres(genre_id, page_no):
	...
	url = '%s/discover/tv?api_key=%s&with_genres=%s&language=en&region=US&with_original_language=en&include_null_first_air_dates=false&release_date.lte=%s&page=%s' \
							% (base_url, tmdb_api_key, genre_id, get_current_date(), page_no)
	return lists_cache_object(get_data, string, url, json=False, expiration=EXPIRY_2_DAYS)

def tmdb_tv_languages(language, page_no):
	string = 'tmdb_tv_languages_%s_%s' % (language, page_no)
	url = '%s/discover/tv?api_key=%s&language=en&include_null_first_air_dates=false&with_original_language=%s&release_date.lte=%s&page=%s' \
							% (base_url, tmdb_api_key, language, get_current_date(), page_no)
	return lists_cache_object(get_data, string, url, json=False, expiration=EXPIRY_2_DAYS)

def tmdb_tv_networks(network_id, page_no):
	string = 'tmdb_tv_networks_%s_%s' % (network_id, page_no)
	url = '%s/discover/tv?api_key=%s&language=en&region=US&with_original_language=en&include_null_first_air_dates=false&with_networks=%s&release_date.lte=%s&page=%s' \
...
def tmdb_tv_year(year, page_no):
	...
	url = '%s/discover/tv?api_key=%s&language=en&region=US&with_original_language=en&include_null_first_air_dates=false&first_air_date_year=%s&page=%s' \
							% (base_url, tmdb_api_key, year, page_no)
	return lists_cache_object(get_data, string, url, json=False, expiration=EXPIRY_2_DAYS)
...
def tmdb_tv_decade(decade, page_no):
	...
	url = '%s/discover/tv?api_key=%s&language=en&region=US&with_original_language=en&include_null_first_air_dates=false&first_air_date.gte=%s' \
...
def tmdb_tv_recommendations(tmdb_id, page_no):
	...
	url = '%s/tv/%s/recommendations?api_key=%s&language=en&region=US&with_original_language=en&page=%s' % (base_url, tmdb_id, tmdb_api_key, page_no)
	return lists_cache_object(get_data, string, url, json=False, expiration=EXPIRY_2_DAYS)

def tmdb_tv_search(query, page_no):
	string = 'tmdb_tv_search_%s_%s' % (query, page_no)
	url = '%s/search/tv?api_key=%s&language=en&include_adult=true&query=%s&page=%s' % (base_url, tmdb_api_key, query, page_no)
	return lists_cache_object(get_data, string, url, json=False, expiration=EXPIRY_2_DAYS)
