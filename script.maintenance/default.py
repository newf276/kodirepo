...
# ######################### CATEGORIES ################################
def CATEGORIES():
    CreateDir('Maintenance','ur', 'maintenance', ADDON_ICON,ADDON_FANART,'', isFolder=True)
    CreateDir('Advanced Settings','ur', 'adv_settings', ADDON_ICON,ADDON_FANART,'')
    CreateDir('Network Speed Test','ur', 'speedtest', ADDON_ICON,ADDON_FANART,'')
    CreateDir('Backup/Restore','ur','backup_restore',ADDON_ICON,ADDON_FANART,'')    
    CreateDir('Addon Settings','ur','settings',ADDON_ICON,ADDON_FANART,'')
...
def FRESHSTART(mode='verbose'):
    if mode != 'silent': select = xbmcgui.Dialog().yesno("Newfs Maintenance", 'Are you absolutely certain you want to wipe this install?' + '\n' + 'All addons EXCLUDING THIS WIZARD will be completely wiped!', yeslabel='Yes',nolabel='No')
    else: select = 1
    if select == 0: return
    elif select == 1:
...
#xbmc.log("maintenance: action: %s" % action, level=xbmc.LOGINFO)
...
if action   == None: CATEGORIES()
    ...
    xbmc.executebuiltin('Runscript("special://home/addons/script.maintenance/resources/lib/modules/speedtest.py")')
