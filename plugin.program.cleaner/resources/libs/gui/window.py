...
def highlight_text(msg):
    ...
    for item in matches:
        ...
        msg = msg.replace(string, '[COLOR deepskyblue]{0}[/COLOR]'.format(string))
    msg = msg.replace('WARNING', '[COLOR yellow]WARNING[/COLOR]').replace('ERROR', '[COLOR deepskyblue]ERROR[/COLOR]').replace('[NL]', '\n').replace(': EXCEPTION Thrown (PythonToCppException) :', '[COLOR deepskyblue]: EXCEPTION Thrown (PythonToCppException) :[/COLOR]')
