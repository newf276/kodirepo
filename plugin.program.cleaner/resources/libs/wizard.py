...
class Wizard:
    ...
    def _prompt_for_wipe(self):
        ...
        if self.dialog.yesno(CONFIG.ADDONTITLE,
                           ...
                           nolabel='[B][COLOR deepskyblue]No[/COLOR][/B]',
    ...
    def build(self, name, over=False):
        ...
        if warning:
            yes_pressed = self.dialog.yesno("{0} - [COLOR deepskyblue]WARNING!![/COLOR]".format(CONFIG.ADDONTITLE), '[COLOR {0}]There is a chance that the skin will not appear correctly'.format(CONFIG.COLOR2) + '\n' + 'When installing a {0} build on a Kodi {1} install'.format(check.check_build(name, 'kodi'), CONFIG.KODIV) + '\n' + 'Would you still like to install: [COLOR {0}]{1} v{2}[/COLOR]?[/COLOR]'.format(CONFIG.COLOR1, name, check.check_build(name, 'version')), nolabel='[B][COLOR deepskyblue]No, Cancel[/COLOR][/B]', yeslabel='[B][COLOR springgreen]Yes, Install[/COLOR][/B]')
        else:
            if over:
                yes_pressed = 1
            else:
                yes_pressed = self.dialog.yesno(CONFIG.ADDONTITLE, '[COLOR {0}]Would you like to Download and Install: '.format(CONFIG.COLOR2) + '[COLOR {0}]{1} v{2}[/COLOR]?[/COLOR]'.format(CONFIG.COLOR1, name, check.check_build(name,'version')), nolabel='[B][COLOR deepskyblue]No, Cancel[/COLOR][/B]', yeslabel='[B][COLOR springgreen]Yes, Install[/COLOR][/B]')
        if yes_pressed:
            ...
            if int(float(percent)) > 0:
                ...
                if int(float(errors)) > 0:
                    yes_pressed = self.dialog.yesno(CONFIG.ADDONTITLE,
                                       ...
                                       nolabel='[B][COLOR deepskyblue]No Thanks[/COLOR][/B]',
    ...
    def gui(self, name, over=False):
        if name == CONFIG.get_setting('buildname'):
            ...
            yes_pressed = self.dialog.yesno("{0} - [COLOR deepskyblue]WARNING!![/COLOR]".format(CONFIG.ADDONTITLE),
    ...
    def theme(self, name, theme='', over=False):
        ...
        if not theme:
            ...
            if response:
                ...
                if len(themes) > 0:
                    if self.dialog.yesno(CONFIG.ADDONTITLE, "[COLOR {0}]The Build [COLOR {1}]{2}[/COLOR] comes with [COLOR {3}]{4}[/COLOR] different themes".format(CONFIG.COLOR2, CONFIG.COLOR1, name, CONFIG.COLOR1, len(themes)) + '\n' + "Would you like to install one now?[/COLOR]",
                                    ...
                                    nolabel="[B][COLOR deepskyblue]Cancel Themes[/COLOR][/B]"):
            ...
            installtheme = self.dialog.yesno(CONFIG.ADDONTITLE, '[COLOR {0}]Would you like to install the theme:'.format(CONFIG.COLOR2) +' \n' + '[COLOR {0}]{1}[/COLOR]'.format(CONFIG.COLOR1, theme) + '\n' + 'for [COLOR {0}]{1} v{2}[/COLOR]?[/COLOR]'.format(CONFIG.COLOR1, name, check.check_build(name,'version')),yeslabel="[B][COLOR springgreen]Install Theme[/COLOR][/B]", nolabel="[B][COLOR deepskyblue]Cancel Themes[/COLOR][/B]")
