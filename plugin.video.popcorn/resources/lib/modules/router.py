...
def dispatch(params):
    ...
    overwrite_cache = params.get("popcorn_reload") == "true"
    ...
    g.log(f"Popcorn, Running Path - {g.REQUEST_PARAMS}")
    ...
    if action is None:
        ...
        try:
            ...
            try:
                popcorn_player = player.PopcornPlayer()
                popcorn_player.play_source(stream_link, item_information, resume_time=resume_time)
            finally:
                if background:
                    try:
                        background.close()
