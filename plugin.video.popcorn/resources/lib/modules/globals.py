...
class GlobalVariables:
    ...
    def init_globals(self, argv=None, addon_id=None):
        ...
        self.DEFAULT_LOGO = f"{self.IMAGES_PATH}logo-popcorn-3.png"
        self.DEFAULT_POSTER = f"{self.IMAGES_PATH}poster-popcorn-3.png"
    ...
    def clear_kodi_bookmarks(self):
        with self.get_kodi_video_db_connection() as video_database:
            if file_ids := [
                str(i["idFile"])
                for i in video_database.fetchall("SELECT * FROM files WHERE strFilename LIKE '%plugin.video.popcorn%'")
            ]:
    ...
    def get_user_text_color(self):
        ...
        :return:Selected color from the user or the default from the skin or the Popcorn default (deepskyblue)
        :rtype:str
        """
    ...
    def is_addon_visible(self):
        return xbmc.getInfoLabel('Container.PluginName') == "plugin.video.popcorn"
    ...
    def open_addon_settings(self, section_offset, setting_offset=None):
        """
        Open popcorn settings at a particular section and setting
