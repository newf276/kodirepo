...
def wipe_install():
    """
    Destroys Popcorn's user_data folder for current user resetting addon to default
    :return: None
    :rtype: None
    """
...
def premiumize_transfer_cleanup():
    """
    Cleanup transfers created by Popcorn at Premiumize
    :return: None
    :rtype: NOne
    """
    ...
    if fair_usage < threshold:
        ...
        return
    popcorn_transfers = premiumize_transfers.get_premiumize_transfers()
    if popcorn_transfers is None:
        g.log("Failed to cleanup transfers, API error", "error")
        return
    if len(popcorn_transfers) == 0:
        g.log("No Premiumize transfers have been created")
        return
    g.log("Premiumize Fair Usage is above threshold, cleaning up Popcorn transfers")
    for i in popcorn_transfers:
