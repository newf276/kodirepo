...
DEFAULT_SKIN_NAME = "Popcorn Default"
...
class SkinManager(Database, ZipManager):
    """
    Class for handling of installed custom themes for Popcorn
    """
    ...
    def __init__(self):
        ...
        self.popcorn_skins = [DEFAULT_SKIN_NAME]
        self.installed_skins = self._get_all_installed()
        if "Popcorn Fox" in [i["skin_name"] for i in self.installed_skins]:
            self.execute_sql("DELETE FROM [skins] where [skin_name] = 'Popcorn Fox'")
    ...
    def _get_active_skin_path(self):
        ...
        if active_skin_name in self.popcorn_skins:
            return g.ADDON_PATH
        else:
    ...
    def _add_skin_to_database(self, skin_meta):
        if skin_meta["skin_name"] in self.popcorn_skins:
    ...
    def install_skin(self, zip_location=None, silent=False):
        """
        Method to install a new theme into Popcorn
